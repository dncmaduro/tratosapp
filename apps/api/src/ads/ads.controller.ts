...
@ApiTags("ads") @ApiBearerAuth() @UseGuards(JwtAuthGuard) @Controller("dailyads")
export class AdsController {
  constructor(@InjectModel(DailyAdsMetrics.name) private readonly metrics: Model<DailyAdsMetricsDocument>) {}
  @Post("metrics") async upsert(@Body() body: any) {
  ...
  @Delete("metrics/delete") async remove(@Body() body: { date: string; channelId: string }) { const date = new Date(body.date); date.setHours(0, 0, 0, 0); await this.metrics.deleteOne({ date, channel: new Types.ObjectId(body.channelId) }); return { success: true } }
}
