...
import { GetAssistantRevenueRankingsResponse } from "../../hooks/models"

interface AssistantRevenueRankingsChartProps {
  isLoadingRankings: boolean
  rankingsData?: GetAssistantRevenueRankingsResponse | null
}

export const AssistantRevenueRankingsChart = ({
  isLoadingRankings,
  rankingsData
}: AssistantRevenueRankingsChartProps) => {
  return (
    <Box>
      {isLoadingRankings ? (
        <Center h={400}>
        ...
        <Paper p="lg" radius="md" withBorder>
          <Stack gap="md">
            <Text size="lg" fw={600}>
              Xếp hạng Trợ live theo Doanh thu
            </Text>
            <BarChart
